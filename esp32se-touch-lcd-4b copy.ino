...
/* ================= CONFIG ================= */
#define FORCE_DEBUG_SCENE true
#define USE_CODE_SCENE true
bool sleep_scene_drawn = false;
lv_obj_t * night_sky_container = NULL;
bool night_sky_built = false;
...
void update_weather_ui(weather_type_t type, bool is_night) {
    if (ui_uiScreenSleep == NULL) return;

    if (!night_sky_built) build_night_sky_once(ui_uiScreenSleep);

    if (FORCE_DEBUG_SCENE) { type = WEATHER_CLEAR; is_night = true; }

    if (USE_CODE_SCENE && type == WEATHER_CLEAR && is_night) {
        // Hide the default PNG object
        if(ui_uiImgBg) lv_obj_add_flag(ui_uiImgBg, LV_OBJ_FLAG_HIDDEN); 
        
        // Show our procedural container
        if(night_sky_container) {
            lv_obj_clear_flag(night_sky_container, LV_OBJ_FLAG_HIDDEN);
            // Re-ensure it is at the bottom so it doesn't cover text
            lv_obj_move_to_index(night_sky_container, 0); 
        }
        
        if(ui_uiIconWeather) {
            lv_obj_clear_flag(ui_uiIconWeather, LV_OBJ_FLAG_HIDDEN);
            lv_obj_set_style_bg_image_src(ui_uiIconWeather, &ui_img_weather_night_png, 0);
        }
        return; 
    }

    // --- FALLBACK TO PNG ---
    if(night_sky_container) lv_obj_add_flag(night_sky_container, LV_OBJ_FLAG_HIDDEN);
    if(ui_uiImgBg) lv_obj_clear_flag(ui_uiImgBg, LV_OBJ_FLAG_HIDDEN);
...
void draw_clear_night_scene(lv_obj_t * parent) {
    lv_obj_clean(parent);
    
    // Background Gradient
    lv_obj_set_style_bg_color(parent, lv_color_hex(0x1A2F4B), 0); 
    lv_obj_set_style_bg_grad_color(parent, lv_color_hex(0x040912), 0); 
    lv_obj_set_style_bg_grad_dir(parent, LV_GRAD_DIR_VER, 0);

    // Stars FIRST (so they are naturally in the background)
    for(int i = 0; i < 40; i++) {
        lv_obj_t * star = lv_obj_create(parent);
        lv_obj_set_size(star, 2, 2);
        lv_obj_set_pos(star, rand() % 480, rand() % 480);
        ...
        lv_obj_set_style_opa(star, (rand() % 155) + 100, 0);
    }

    // Moon LAST (so it's on top)
    lv_obj_t * moon = lv_obj_create(parent);
    lv_obj_set_size(moon, 150, 150);
    lv_obj_align(moon, LV_ALIGN_TOP_RIGHT, 30, -30);
    lv_obj_set_style_radius(moon, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(moon, lv_color_hex(0xF2C94C), 0);
    lv_obj_set_style_border_width(moon, 0, 0);
    
    // Reduced shadow complexity to prevent hanging
    lv_obj_set_style_shadow_width(moon, 80, 0);
    lv_obj_set_style_shadow_color(moon, lv_color_hex(0xF2C94C), 0);
    lv_obj_set_style_shadow_opa(moon, LV_OPA_30, 0);
}

void build_night_sky_once(lv_obj_t * parent) {
    if (night_sky_built) return;

    // Build this on the Sleep Screen directly
    night_sky_container = lv_obj_create(ui_uiScreenSleep); 
    lv_obj_set_size(night_sky_container, 480, 480);
    
    // remove default border/padding that might cause black gaps
    lv_obj_set_style_pad_all(night_sky_container, 0, 0);
    lv_obj_set_style_border_width(night_sky_container, 0, 0);
    lv_obj_set_style_radius(night_sky_container, 0, 0);

    // Force it to the background so Clock/Date labels stay on top
    lv_obj_move_to_index(night_sky_container, 0); 

    // Gradient Background
    lv_obj_set_style_bg_color(night_sky_container, lv_color_hex(0x0F172A), 0);
    lv_obj_set_style_bg_grad_color(night_sky_container, lv_color_hex(0x040912), 0); 
    lv_obj_set_style_bg_grad_dir(night_sky_container, LV_GRAD_DIR_VER, 0);

    // Stars FIRST
    for(int i = 0; i < 30; i++) {
        lv_obj_t * s = lv_obj_create(night_sky_container);
        lv_obj_set_size(s, 2, 2);
        lv_obj_set_pos(s, rand() % 480, rand() % 480);
        lv_obj_set_style_bg_color(s, lv_color_hex(0xFFFFFF), 0);
        lv_obj_set_style_radius(s, LV_RADIUS_CIRCLE, 0);
        lv_obj_set_style_border_width(s, 0, 0);
        lv_obj_set_style_opa(s, 200, 0);
    }

    // Moon Body
    lv_obj_t * moon = lv_obj_create(night_sky_container);
    lv_obj_set_size(moon, 140, 140);
    lv_obj_align(moon, LV_ALIGN_TOP_RIGHT, -20, 20); // Moved inward slightly
    lv_obj_set_style_radius(moon, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(moon, lv_color_hex(0xF2C94C), 0);
    lv_obj_set_style_border_width(moon, 0, 0);
    ...
    lv_obj_add_flag(night_sky_container, LV_OBJ_FLAG_HIDDEN); 
    night_sky_built = true;
}
